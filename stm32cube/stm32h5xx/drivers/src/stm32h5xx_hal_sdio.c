...
HAL_StatusTypeDef HAL_SDIO_Init(SDIO_HandleTypeDef* hsdio) {
    ...
    sts  = SDIO_WriteDirect(hsdio, SDMMC_SDIO_CCCR4_SD_BYTE3,
...
HAL_StatusTypeDef HAL_SDIO_SetSpeedMode(SDIO_HandleTypeDef* hsdio, uint32_t DataRate) {
    ...
    switch (DataRate) {
        case HAL_SDIOS_DATA_RATE_SDR25 :
            ...
            errorstate =
                SDIO_WriteDirect(hsdio, SDMMC_SDIO_CCCR16_SD_BYTE3, HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0, &data);
            break;

        case HAL_SDIOS_DATA_RATE_SDR50 :
            data       = SDIO_BUS_SPEED_SDR50;
            errorstate = SDIO_WriteDirect(hsdio,
        ...
        case HAL_SDIOS_DATA_RATE_DDR50 :
            ...
            errorstate = SDIO_WriteDirect(hsdio,
                                          ((SDIO_FUNCTION_0 << 2) | (SDIO_FUNCTION_0 << 1) | (SDIO_FUNCTION_0 << 14) |
...
HAL_StatusTypeDef HAL_SDIO_CardReset(SDIO_HandleTypeDef* hsdio) {
     ...
     */
    sts = SDIO_WriteDirect(hsdio,
                           ((SDIO_FUNCTION_0 << 2) | (SDIO_FUNCTION_0 << 1) | (SDIO_FUNCTION_0 << 14) | SDMMC_SDIO_CCCR4_SD_BYTE2),
                           HAL_SDIO_WRITE_ONLY, 0U, &data);
    if (sts != HAL_OK) {
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef HAL_SDIO_GetCardCommonControlRegister(SDIO_HandleTypeDef* hsdio, HAL_SDIO_CCCR_TypeDef* pCccr) {
    ...
    for (count = 0U; count <= SDIO_CCCR_REG_NUMBER; count++) {
        if (SDIO_ReadDirect(hsdio, SDMMC_SDIO_CCCR0 + count, HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0,
                            &tempBuffer[count]) != HAL_OK) {
            return HAL_ERROR;
        }
    }
    ...
    pCccr->bus_width_8Bit =
        ((tempBuffer[0x07U] & 0x04U) != 0U) ? HAL_SDIO_BUS_WIDTH_8BIT_SUPPORTED : HAL_SDIO_BUS_WIDTH_8BIT_NOT_SUPPORTED;
...
HAL_StatusTypeDef HAL_SDIO_GetCardFBRRegister(SDIO_HandleTypeDef* hsdio, HAL_SDIO_FBR_t* pFbr) {
    ...
    for (func_idx = 2U; func_idx <= SDIO_MAX_IO_NUMBER; func_idx++) {
        for (count = 0U; count <= SDIO_CCCR_REG_NUMBER; count++) {
            if (SDIO_ReadDirect(hsdio, (((uint32_t)SDMMC_SDIO_F1BR0 * (uint32_t)func_idx) + count), HAL_SDIO_WRITE_ONLY,
                                SDIO_FUNCTION_0, &tempBuffer[count]) != HAL_OK) {
                return HAL_ERROR;
            }
        }
        pFbr[(uint32_t)func_idx - 1U].ioStdFunctionCode = tempBuffer[0U] & 0x0FU;
        pFbr[(uint32_t)func_idx - 1U].ioExtFunctionCode = tempBuffer[1U];
        pFbr[(uint32_t)func_idx - 1U].ioPointerToCIS =
            tempBuffer[9U] | ((uint32_t)tempBuffer[10U] << 8U) | ((uint32_t)tempBuffer[11U] << 16U);
        pFbr[(uint32_t)func_idx - 1U].ioPointerToCSA =
...
HAL_StatusTypeDef HAL_SDIO_ReadExtended(SDIO_HandleTypeDef* hsdio, HAL_SDIO_ExtendedCmd_TypeDef* Argument,
    ...
    if (hsdio->State == HAL_SDIO_STATE_READY) {
        ...
        /* (HAL_SDIO_MODE_BLOCK << 27) corresponds to the block mode bit of the CMD argument */
        config.TransferMode =
            (Argument->Block_Mode == HAL_SDIO_MODE_BLOCK) ? SDMMC_TRANSFER_MODE_BLOCK : SDMMC_TRANSFER_MODE_SDIO;
        ...
        while (!__HAL_SDIO_GET_FLAG(hsdio, SDMMC_FLAG_RXOVERR | SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_DTIMEOUT |
                                               SDMMC_FLAG_DATAEND)) {
...
HAL_StatusTypeDef HAL_SDIO_ReadExtended_DMA(SDIO_HandleTypeDef* hsdio, HAL_SDIO_ExtendedCmd_TypeDef* Argument,
    ...
    if (hsdio->State == HAL_SDIO_STATE_READY) {
        ...
        if (nbr_of_block != 0U) {
            ...
            hsdio->next_data_addr +=
                (Size_byte < HAL_SDIO_DATA_BLOCK_SIZE_512BYTE) ? Size_byte : HAL_SDIO_DATA_BLOCK_SIZE_512BYTE;
            if (hsdio->remaining_data != 0U) {
                ...
                hsdio->next_reg_addr += (Size_byte >= HAL_SDIO_DATA_BLOCK_SIZE_512BYTE)
                                            ? (HAL_SDIO_DATA_BLOCK_SIZE_512BYTE + 1U)
                                            : (Size_byte + 1U);
            }
        }
...
HAL_StatusTypeDef HAL_SDIO_WriteExtended_DMA(SDIO_HandleTypeDef* hsdio, HAL_SDIO_ExtendedCmd_TypeDef* Argument,
    ...
    if (hsdio->State == HAL_SDIO_STATE_READY) {
        ...
        if (nbr_of_block != 0U) {
            ...
            if (hsdio->remaining_data != 0U) {
                hsdio->remaining_data = (Size_byte >= HAL_SDIO_DATA_BLOCK_SIZE_512BYTE)
                                            ...
                                            : (Size_byte - hsdio->remaining_data);
                hsdio->next_reg_addr +=
                    ((Size_byte >= HAL_SDIO_DATA_BLOCK_SIZE_512BYTE) ? (HAL_SDIO_DATA_BLOCK_SIZE_512BYTE >> 1U)
                                                                     : (Size_byte >> 1U)) |
        ...
        if (Argument->Block_Mode == HAL_SDIO_MODE_BLOCK) {
            ...
            config.DataLength =
                (Size_byte > HAL_SDIO_DATA_BLOCK_SIZE_512BYTE) ? HAL_SDIO_DATA_BLOCK_SIZE_512BYTE : Size_byte;
            config.DataBlockSize = SDMMC_DATABLOCK_SIZE_1B;
        }
        ...
        /* Write */
        hsdio->Context = (uint32_t)((Argument->Block_Mode == HAL_SDIO_MODE_BLOCK) ? SDIO_CONTEXT_WRITE_MULTIPLE_BLOCK
                                                                                  : SDIO_CONTEXT_WRITE_SINGLE_BLOCK) |
                         SDIO_CONTEXT_DMA;
        cmd = SDIO_WRITE << 31U;
        cmd |= Argument->IOFunctionNbr << 28U;
 ...
 * @retval HAL state
 */
HAL_SDIO_StateTypeDef HAL_SDIO_GetState(SDIO_HandleTypeDef const* hsdio) {
    return hsdio->State;
}
 ...
 */
uint32_t HAL_SDIO_GetError(SDIO_HandleTypeDef const* hsdio) {
...
HAL_StatusTypeDef HAL_SDIO_EnableIOFunctionInterrupt(SDIO_HandleTypeDef* hsdio, uint32_t IOFunction) {
    ...
    if (SDIO_WriteDirect(hsdio, SDMMC_SDIO_CCCR4, HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0, &intEn) != HAL_OK) {
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef HAL_SDIO_DisableIOFunctionInterrupt(SDIO_HandleTypeDef* hsdio, uint32_t IOFunction) {
    ...
    if (SDIO_WriteDirect(hsdio, SDMMC_SDIO_CCCR4, HAL_SDIO_READ_AFTER_WRITE, SDIO_FUNCTION_0, &intEn) != HAL_OK) {
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef HAL_SDIO_EnableIOFunction(SDIO_HandleTypeDef* hsdio, uint32_t IOFunction) {
    ...
    if (SDIO_WriteDirect(hsdio, SDMMC_SDIO_CCCR0_SD_BYTE2, HAL_SDIO_READ_AFTER_WRITE, SDIO_FUNCTION_0, &ioEn) !=
        HAL_OK) {
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef HAL_SDIO_DisableIOFunction(SDIO_HandleTypeDef* hsdio, uint32_t IOFunction) {
    ...
    sts = SDIO_ReadDirect(hsdio, SDMMC_SDIO_CCCR0_SD_BYTE2, HAL_SDIO_WRITE_ONLY, SDIO_FUNCTION_0, &ioEn);
    if (sts != HAL_OK) {
        return HAL_ERROR;
    }
    ...
    sts = SDIO_WriteDirect(hsdio, SDMMC_SDIO_CCCR0_SD_BYTE2,
                           HAL_SDIO_READ_AFTER_WRITE, SDIO_FUNCTION_0,
                           &ioEn);
    if (sts != HAL_OK) {
        return HAL_ERROR;
    }
...
static HAL_StatusTypeDef SDIO_WriteExtended(SDIO_HandleTypeDef* hsdio, HAL_SDIO_ExtendedCmd_TypeDef* cmd_arg,
    ...
    config.TransferDir = SDMMC_TRANSFER_DIR_TO_CARD;
    config.TransferMode =
        (cmd_arg->Block_Mode == HAL_SDIO_MODE_BLOCK) ? SDMMC_TRANSFER_MODE_BLOCK : SDMMC_TRANSFER_MODE_SDIO;
    config.DPSM = SDMMC_DPSM_DISABLE;
    (void)SDMMC_ConfigData(hsdio->Instance, &config);
    __SDMMC_CMDTRANS_ENABLE(hsdio->Instance);
    ...
    dataremaining = config.DataLength;
    while (!__HAL_SDIO_GET_FLAG(hsdio,
        ...
        if (__HAL_SDIO_GET_FLAG(hsdio, SDMMC_FLAG_TXFIFOHE) && (dataremaining >= 32U)) {
            ...
            dataremaining -= 32U;
                 ...
                 (__HAL_SDIO_GET_FLAG(hsdio, SDMMC_FLAG_TXFIFOHE | SDMMC_FLAG_TXFIFOE))) {
            uint8_t* u8buff = (uint8_t*)u32tempbuff;
            while (dataremaining > 0U) {
                data = 0U;
                for (byteCount = 0U; (byteCount < 4U) && (dataremaining > 0U); byteCount++) {
...
static HAL_StatusTypeDef SDIO_IOFunction_IRQHandler(SDIO_HandleTypeDef* hsdio) {
    ...
    if (hsdio->IOInterruptNbr == 1U) {
        ...
        sts = SDIO_ReadDirect(hsdio, SDMMC_SDIO_CCCR4_SD_BYTE1, HAL_SDIO_WRITE_ONLY,
                              SDIO_FUNCTION_0, &pendingInt);
        if (sts != HAL_OK) {
            return HAL_ERROR;
        }
        ...
        if ((pendingInt != 0U) && (hsdio->IOFunctionMask != 0U)) {
            for (count = 1; count <= SDIO_MAX_IO_NUMBER; count++) {
                if (((pendingInt & (1U << count)) != 0U) &&
                    (((1U << count) & hsdio->IOFunctionMask) != 0U)) {
