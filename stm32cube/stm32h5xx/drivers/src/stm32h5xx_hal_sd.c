...
HAL_StatusTypeDef HAL_SD_Init(SD_HandleTypeDef* hsd) {
    ...
    Status = HAL_SD_ConfigWideBusOperation(hsd, hsd->Init.BusWide);
    if (Status != HAL_OK) {
        return HAL_ERROR;
    }
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SD_WriteBlocks(SD_HandleTypeDef* hsd, uint8_t const* pData, uint32_t BlockAdd,
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SD_WriteBlocks_IT(SD_HandleTypeDef* hsd, uint8_t const* pData, uint32_t BlockAdd,
                                        uint32_t NumberOfBlocks) {
    SDMMC_DataInitTypeDef config;
    uint32_t errorstate;
    uint32_t add = BlockAdd;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SD_WriteBlocks_DMA(SD_HandleTypeDef* hsd, uint8_t const* pData, uint32_t BlockAdd,
                                         uint32_t NumberOfBlocks) {
    SDMMC_DataInitTypeDef config;
    uint32_t errorstate;
    uint32_t add = BlockAdd;
 ...
 * @retval HAL state
 */
HAL_SD_StateTypeDef HAL_SD_GetState(SD_HandleTypeDef const* hsd) {
    return hsd->State;
}
 ...
 */
uint32_t HAL_SD_GetError(SD_HandleTypeDef const* hsd) {
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SD_GetCardCID(SD_HandleTypeDef const* hsd, HAL_SD_CardCIDTypeDef* pCID) {
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SD_GetCardInfo(SD_HandleTypeDef const* hsd, HAL_SD_CardInfoTypeDef* pCardInfo) {
    ...
    if (hsd->Init.TranceiverPresent == SDMMC_TRANSCEIVER_PRESENT) {
        switch (SpeedMode) {
            ...
            case SDMMC_SPEED_MODE_HIGH : {
                if ((hsd->SdCard.CardSpeed == CARD_ULTRA_HIGH_SPEED) || (hsd->SdCard.CardSpeed == CARD_HIGH_SPEED) ||
...
static uint32_t SD_SendSDStatus(SD_HandleTypeDef* hsd, uint32_t* pSDstatus) {
    ...
    /* Get status data */
    while (!__HAL_SD_GET_FLAG(hsd, (SDMMC_FLAG_RXOVERR  | SDMMC_FLAG_DCRCFAIL |
